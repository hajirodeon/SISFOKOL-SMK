	...
	if (empty($filex_namex2))
		...
		if ($ext_filex == ".xls")
			...
			for ($i=2; $i<=$baris; $i++)
				...
				$i_nr = nosql($data->val($i, 19));
				$i_nr_a = balikin($data->val($i, 20));
				$i_nr_p = balikin($data->val($i, 21));
				$i_catatan = balikin($data->val($i, 22));
				...
				if ($tcc != 0)
					...
					//jika blm ada, insert
						...
						mysql_query("INSERT INTO siswa_nilai_raport(kd, kd_siswa_kelas, kd_smt, kd_prog_pddkn, ".
										...
										"nil_raport_ketrampilan, nil_raport_ketrampilan_a, nil_raport_ketrampilan_p, nil_k_ketrampilan, postdate) VALUES ".
										...
										"'$i_nr', '$i_nr_a', '$i_nr_p', '$i_catatan', '$today')");
						}
					}
				}
