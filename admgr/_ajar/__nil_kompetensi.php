...
*/
	...
	//re-direct
	$ke = "$filenya?tapelkd=$tapelkd&smtkd=$smtkd&kelkd=$kelkd&".
		"keahkd=$keahkd&mmkd=$mmkd&mpkd=$mpkd&kompkd=$kompkd&page=$page";
	xloc($ke);
	exit();
	}
...
do
	...
	echo '<option value="'.$filenya.'?mmkd='.$mmkd.'&tapelkd='.$tapelkd.'&kelkd='.$kelkd.'&keahkd='.$keahkd.'&mpkd='.$mpkd.'&kompkd='.$kompkd.'&smtkd='.$st_kd.'">'.$st_smt.'</option>';
	}
...
//						"WHERE kd_prog_pddkn_kelas = '$hi_mpkd' ".
			...
			echo '<td width="10" align="center"><strong>NS</strong></td>
			<td width="10" align="center"><strong>NK</strong></td>
			</tr>';
...
//							"AND kd_prog_pddkn = '$pelx_mpkd'");
				...
				echo '<td>
				<input name="ns'.$i_nomer.'" type="text" value="'.$dtu2_ns.'" size="3" maxlength="5">
				</td>
				<td>
