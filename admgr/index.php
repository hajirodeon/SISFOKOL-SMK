...
if (empty($tapelkd))
	...
	if ($ku_no == "1")
		...
		<td width="50"><strong>E-Learning</strong></td>
		</tr>';
		...
		if ($tdty != 0)
			...
			do
				...
				<td>'.$dty_pel.'</td>
				<td>
				<a href="ajar/nil_absensi.php?mmkd='.$dty_gurkd.'&tapelkd='.$dty_tapelkd.'&kelkd='.$dty_kelkd.'&progkd='.$dty_pelkd.'"
				title="Kelas = '.$ykel_kelas.', Pelajaran = '.$dty_pel.'">
				<img src="'.$sumber.'/img/edit.gif" width="16" height="16" border="0"></a>
				</td>
				<td>
				<a href="ajar/nil_raport.php?mmkd='.$dty_gurkd.'&tapelkd='.$dty_tapelkd.'&kelkd='.$dty_kelkd.'&progkd='.$dty_pelkd.'&mpkd='.$pelx_mpkd.'&kompkd='.$dty_kompkd.'"
				title="Kelas = '.$ykel_kelas.', Pelajaran = '.$dty_pel.'">
				<img src="'.$sumber.'/img/edit.gif" width="16" height="16" border="0"></a>
				</td>
				<td>
				<a href="'.$sumber.'/janissari/e/gr/mapel.php?tapelkd='.$dty_tapelkd.'&kelkd='.$dty_kelkd.'&smtkd='.$smtkd.'&progkd='.$dty_pelkd.'&s=detail&gmkd='.$dty_pelkd.'"
				title="Kelas = '.$ykel_kelas.', Pelajaran = '.$dty_pel.'">
				<img src="'.$sumber.'/img/edit.gif" width="16" height="16" border="0"></a>
				</td>
				</tr>';
	...
	//jika kurikulum Kur13
	else if ($ku_no == "2")
		{
		//data ne
		$qdty = mysql_query("SELECT m_guru.*, m_guru_prog_pddkn.*, m_guru_prog_pddkn.kd AS mgkd, ".
								"m_prog_pddkn.kd AS mpkd ".
								"FROM m_guru, m_guru_prog_pddkn, m_prog_pddkn, m_kelas ".
								"WHERE m_guru_prog_pddkn.kd_prog_pddkn = m_prog_pddkn.kd ".
								"AND m_guru_prog_pddkn.kd_guru = m_guru.kd ".
		...
		if ($tdty != 0)
			...
			do
				...
				$rpelx2 = mysql_fetch_assoc($qpelx2);
				$pelx_mpkd = nosql($rpelx2['mpkd']);
				$dty_pel = balikin($rpelx2['prog_pddkn']);
				...
				<img src="'.$sumber.'/img/edit.gif" width="16" height="16" border="0"></a>
				<br>';
				$qx = mysql_query("SELECT postdate FROM guru_silabus ".
				...
				<img src="'.$sumber.'/img/edit.gif" width="16" height="16" border="0"></a>
				<br>';
				$qx = mysql_query("SELECT postdate FROM guru_rpp ".
										"WHERE kd_guru_prog_pddkn = '$dty_gurkd' ".
										"ORDER BY postdate DESC");
				$rowx = mysql_fetch_assoc($qx);
				$e_postdate = $rowx['postdate'];
				...
				<br>';
				$qx = mysql_query("SELECT postdate FROM m_prog_pddkn_deskripsi ".
				...
				$qx = mysql_query("SELECT tanggal FROM siswa_nilai_absensi ".
				...
				$qx = mysql_query("SELECT postdate FROM siswa_nilai_raport ".
										"WHERE kd_prog_pddkn = '$dty_pelkd' ".
