...
if ($_POST['btnSMP'])
	...
	do
		...
		//jika gak null, masukkan ke rekap bp/bk
		if (!empty($xnhx))
			{
			//cek
			$qcc = mysql_query("SELECT * FROM siswa_absensi ".
									"WHERE kd_siswa_kelas = '$i_skkd' ".
									"AND round(DATE_FORMAT(tgl, '%d')) = '$utgl' ".
			...
			if ($tcc != 0)
				{
				//update
				mysql_query("UPDATE siswa_absensi SET kd_absensi = '$xnhx' ".
								"WHERE kd_siswa_kelas = '$i_skkd' ".
								"AND round(DATE_FORMAT(tgl, '%d')) = '$utgl' ".
								"AND round(DATE_FORMAT(tgl, '%m')) = '$ubln' ".
								"AND round(DATE_FORMAT(tgl, '%Y')) = '$uthn'");
				...
				{
				mysql_query("INSERT INTO siswa_absensi(kd, kd_siswa_kelas, kd_absensi, tgl, postdate) VALUES ".
								"('$x', '$i_skkd', '$xnhx', '$tgl_absen', '$today')");
				}
			}
