...
*/
	...
	$sqlcount = "SELECT m_siswa.kd AS mskd ".
	...
	do
		...
		do
			...
			$q33 = mysql_query("SELECT * FROM siswa_sikap_observasi ".
									"WHERE kd_tapel = '$tapelkd' ".
									"AND kd_kelas = '$kelkd' ".
									"AND kd_mapel = '$progkd' ". 
									"AND kd_siswa = '$i_kd' ".
									"AND kd_ket = '$kd'");
			$row33 = mysql_fetch_assoc($q33);
			$total33 = mysql_num_rows($q33);
				
			//jika ada
			if (!empty($total33))
				{
				//update
				mysql_query("UPDATE siswa_sikap_observasi SET pilihan = '$pilnil', ".
								"postdate = '$today' ".
								"WHERE kd_tapel = '$tapelkd' ".
								"AND kd_kelas = '$kelkd' ".
								"AND kd_mapel = '$progkd' ".
...
if (empty($tapelkd))
	...
	if ($count != 0)
		...
		do
			...
			<input name="swkd'.$nomer.'" type="hidden" value="'.$i_kd.'">
			'.$i_nis.'
			</td>
			...
			do
				...
				$ii_kd = nosql($row['kd']);
				$ii_no = nosql($row['no']);
				$ii_nama = balikin($row['nama']);



				//munculkan
				$q2 = mysql_query("SELECT * FROM siswa_sikap_observasi ".
									"WHERE kd_tapel = '$tapelkd' ".
									"AND kd_kelas = '$kelkd' ".
									"AND kd_mapel = '$progkd' ".
									"AND kd_ket = '$ii_kd' ". 
				...
				$ii_pilihan = balikin($row2['pilihan']);
							
							
				//jika satu
				if ($ii_pilihan == "1")
					{
					$ii_pilihann = "Tidak Pernah";
					}	
				else if ($ii_pilihan == "2")
					{
					$ii_pilihann = "Kadang - kadang";
					}	
				else if ($ii_pilihan == "3")
					{
					$ii_pilihann = "Sering";
					}	
				else if ($ii_pilihan == "4")
					{
					$ii_pilihann = "Selalu";
					}	
				else 
				...
				echo '<td valign="top">
				<input name="'.$i_kd.'pilkd'.$ii_kd.'" type="hidden" value="'.$ii_kd.'">
				<select name="'.$i_kd.'pil'.$ii_kd.'">
				<option value="'.$ii_pilihan.'" selected>'.$ii_pilihann.'</option>
