...
if ($_POST['btnBTL'])
	...
	//re-direct
	$ke = "$filenya?keahkd=$keahkd&kompkd=$kompkd";
	xloc($ke);
	exit();
	}
	...
	if ((empty($kode)) OR (empty($nama)))
		...
		if (empty($s))
			...
			if ($tcc != 0)
				...
				{
				//query
				mysql_query("INSERT INTO inv_brg(kd, kd_keahlian, kd_keahlian_kompetensi, kode, nama) VALUES ".
						"('$x', '$keahkd', '$kompkd', '$kode', '$nama')");
		...
		//jika update
			...
			//re-direct
			$ke = "$filenya?keahkd=$keahkd&kompkd=$kompkd&page=$page";
			xloc($ke);
			exit();
			}
		}
...
if ($_POST['btnSMP2'])
	...
	if ((empty($no_seri)) OR (empty($merk)) OR (empty($model)))
		...
		//re-direct
		$pesan = "Input Tidak Lengkap. Harap Diulangi...!!";
		$ke = "$filenya?keahkd=$keahkd&kompkd=$kompkd&page=1&s=ada&brgkd=$brgkd";
		pekem($pesan,$ke);
		exit();
		}
	else
...
if ($_POST['btnSMP3'])
	...
	if (empty($jml_bagus))
		...
		//re-direct
		$pesan = "Input Tidak Lengkap. Harap Diulangi...!!";
		$ke = "$filenya?keahkd=$keahkd&kompkd=$kompkd&page=1&s=stock&brgkd=$brgkd";
		pekem($pesan,$ke);
		exit();
		}
	else
...
if ($_POST['btnHPS2'])
	...
	//auto-kembali
	$ke = "$filenya?keahkd=$keahkd&kompkd=$kompkd&s=ada&brgkd=$brgkd&page=$page";
	xloc($ke);
	exit();
	}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
if ((empty($s)) OR ($s == "edit"))
	...
	if ($count != 0)
		...
		<td width="200"><strong><font color="'.$warnatext.'">Stock</font></strong></td>
		</tr>';
		...
		do
			...
			<td>
			<a href="'.$filenya.'?keahkd='.$keahkd.'&kompkd='.$kompkd.'&page='.$page.'&s=edit&brgkd='.$i_kd.'">
			...
			<td>'.$i_nama.'</td>
			<td>
			<a href="'.$filenya.'?keahkd='.$keahkd.'&kompkd='.$kompkd.'&page='.$page.'&s=ada&brgkd='.$i_kd.'" title="Pengadaan : '.$i_kode.'.'.$i_nama.'">
			<img src="'.$sumber.'/img/edit.gif" width="16" height="16" border="0">
			</a>
			</td>
			<td>
			<a href="'.$filenya.'?keahkd='.$keahkd.'&kompkd='.$kompkd.'&page='.$page.'&s=stock&brgkd='.$i_kd.'" title="Stock : '.$i_kode.'.'.$i_nama.'">
			<img src="'.$sumber.'/img/edit.gif" width="16" height="16" border="0">
			</a>
			[Total : <strong>'.$daa_jml.'</strong>].
			<br>
			[Bagus : <strong>'.$sto_jml_bagus.'</strong>].
			[Sedang : <strong>'.$sto_jml_sedang.'</strong>].
			<br>
			[Rusak : '.$sto_jml_rusak.'].
			[Hilang : '.$sto_jml_hilang.'].
			</td>
			</tr>';
			}
		while ($data = mysql_fetch_assoc($result));
