...
if ($_POST['btnBTL'])
	...
	//re-direct
	$ke = "$filenya?keahkd=$keahkd&kompkd=$kompkd&jnskd=$jnskd";
	xloc($ke);
	exit();
	}
...
echo '<strong>'.$tpx_kompetensi.'</strong>
</td>
</tr>
</table>
...
do
	...
	echo '<option value="'.$filenya.'?keahkd='.$keahkd.'&kompkd='.$kompkd.'&jnskd='.$tp_kd.'">'.$tp_jns.'</option>';
	}
...
if (empty($keahkd))
	...
	if ($total != 0)
		...
		do {
			...
			<td>
			<a href="'.$filenya.'?keahkd='.$keahkd.'&kompkd='.$kompkd.'&s=edit&jnskd='.$jnskd.'&kd='.$kd.'">
