...
if ($_POST['btnBTL'])
	...
	//re-direct
	$ke = "nil_pengetahuan.php?tapelkd=$tapelkd&kelkd=$kelkd&progkd=$progkd&jnskd=$jnskd&smtkd=$smtkd";
	xloc($ke);
	exit();
	}
...
if ($_POST['btnIMx'])
	...
	if (empty($filex_namex2))
		...
		$ke = "nil_pengetahuan.php?tapelkd=$tapelkd&kelkd=$kelkd&progkd=$progkd&smtkd=$smtkd&jnskd=$jnskd&s=import";
		pekem($pesan,$ke);
		exit();
		}
	else
		...
		if ($ext_filex == ".xls")
			...
			for ($i=2; $i<=$baris; $i++)
				...
				$i_nil_raport_a = balikin($data->val($i, 18));
				$i_nil_raport_p = balikin($data->val($i, 19));
				$i_catatan = nosql($data->val($i, 20));
