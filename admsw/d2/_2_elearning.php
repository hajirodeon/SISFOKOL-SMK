...
do
	...
	$quru = mysql_query("SELECT m_pegawai.* ".
							...
							"AND m_guru_prog_pddkn.kd_prog_pddkn = '$std_kd'");
	$ruru = mysql_fetch_assoc($quru);
	$uru_kd = nosql($ruru['kd']);
	$uru_nip = nosql($ruru['nip']);
	$uru_nama = balikin($ruru['nama']);

	
	//lakukan update
	mysql_query("UPDATE guru_mapel SET kd_user = '$uru_kd' ".
					"WHERE kd = '$std_kd'");
	



	echo '<option value="'.$sumber.'/janissari/e/sw/mapel.php?tapelkd='.$tapelkd.'&kelkd='.$kelkd.'&keahkd='.$keahkd.'&kompkd='.$kompkd.'&smtkd='.$smtkd.'&progkd='.$std_kd.'&s=detail&gmkd='.$std_kd.'&pegkd='.$uru_kd.'">'.$std_pel.' ['.$uru_nip.']. ['.$uru_nama.'].</option>';
	}
