	...
	if (empty($filex_namex))
		...
		if ($ext_filex == ".xls")
			...
			for ($i=2; $i<=$baris+5; $i++)
				...
				$i_pegawai_karpeg = $data->val($i,29);

				$i_kerja_pangkat = $data->val($i,30);
				...
				$i_sertifikasi = $data->val($i,34);
				$i_sertifikasi_tahun = $data->val($i,35);
				$i_email = $data->val($i,36);
				...
				if ($tcc != 0)
					...
					mysql_query("UPDATE m_pegawai SET usernamex = '$i_pegawai_nip', ".
									...
									"sertifikasi_tahun = '$i_sertifikasi_tahun', ".						
									"email = '$i_email' ".
									"WHERE kd = '$cc_kd'");
				...
				//jika blm ada, insert
					...
					mysql_query("INSERT INTO m_pegawai(kd, usernamex, passwordx, nip, nuptk, nama, ".
									...
									"alamat, telp, gol_darah, sertifikasi, sertifikasi_tahun, email, postdate) VALUES ".
					...
					mysql_query("INSERT INTO m_user(kd, usernamex, passwordx, nomor, nama, tipe, postdate) VALUES ".
									"('$i_xyz', '$i_pegawai_nip', '$i_pass', '$i_pegawai_nip', '$i_nama', 'PEGAWAI', '$today')");
					}
