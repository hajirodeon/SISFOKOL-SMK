...
if(isset($_GET['aksi']) && $_GET['aksi'] == 'simpan')
	...
	$menu = cegah2($_GET['e_nama']);

	//nek null
	if ((empty($kode)) OR (empty($menu)))
		{
		echo '<p>
		<b>
		<font color="red">
		...
		{
		//cek
		$qcc = mysql_query("SELECT * FROM cp_m_menu ".
								"WHERE no = '$kode'");
...
if(isset($_GET['aksi']) && $_GET['aksi'] == 'simpan2')
	...
	if ((empty($kode)) OR (empty($menu)))
		...
		{
		//query
		mysql_query("UPDATE cp_m_menu SET no = '$kode', ".
						"nama = '$menu' ".
						"WHERE kd = '$kd'");
