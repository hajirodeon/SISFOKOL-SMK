...
if ($s == "edit")
	...
	//query
	$qx = mysql_query("SELECT * FROM m_prog_pddkn_jns ".
						"WHERE kd = '$kdx'");
	$rowx = mysql_fetch_assoc($qx);
	$e_no = nosql($rowx['no']);
	$e_jenis = balikin2($rowx['jenis']);
	}
...
if ($_POST['btnSMP'])
	...
	if ((empty($no)) OR (empty($jenis)))
		...
		{ ///cek
		$qcc = mysql_query("SELECT * FROM m_prog_pddkn_jns ".
								"WHERE jenis = '$jenis'");
		...
		if ($tcc != 0)
			...
			//re-direct
			$pesan = "jenis Tersebut Sudah Ada. Silahkan Ganti Yang Lain...!!";
			pekem($pesan,$filenya);
			exit();
			}
		else
			...
			if (empty($s))
				{
				//query
				mysql_query("INSERT INTO m_prog_pddkn_jns(kd, no, jenis) VALUES ".
								"('$x', '$no', '$jenis')");
			...
			//jika update
				...
				{
				//query
				mysql_query("UPDATE m_prog_pddkn_jns SET no = '$no', ".
								"jenis = '$jenis' ".
								"WHERE kd = '$kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysql_query("DELETE FROM m_prog_pddkn_jns ".
						"WHERE kd = '$kd'");
		}
...
//query
$q = mysql_query("SELECT * FROM m_prog_pddkn_jns ".
...
jenis :
<br>
<input name="jenis" type="text" value="'.$e_jenis.'" size="30">
...
if ($total != 0)
	...
	<td><strong><font color="'.$warnatext.'">Jenis Mapel</font></strong></td>
	</tr>';
