...
//nilai
$filenya = "siswa_lab.php";
$judul = "Keuangan Siswa : Uang Lab.Bahasa";
...
if ($s == "hapus")
	...
	//query
	mysql_query("UPDATE siswa_uang_lab SET lunas = 'false', ".
...
if ($_POST['btnBTL'])
	...
	//cek
	$qswp = mysql_query("SELECT * FROM siswa_uang_lab ".
				"WHERE kd_siswa = '$swkd' ".
...
if ($_POST['btnSMP'])
	...
	do
		...
		//update
		mysql_query("UPDATE siswa_uang_lab SET lunas = 'true', ".
				"tgl_bayar = '$today', ".
				"postdate = '$today3', ".
				"nilai = '$nil_uang' ".
				"WHERE kd_tapel = '$tapelkd' ".
	...
	//re-direct
	$ke = "siswa_lab_prt.php?tapelkd=$tapelkd&nis=$nis";
	xloc($ke);
	exit();
	}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if (!empty($nis))
		...
		for ($i=1;$i<=12;$i++)
			...
			if ($i<=6) //bulan juli sampai desember
				...
				//cek
				$qccu = mysql_query("SELECT * FROM siswa_uang_lab ".
				...
				if (empty($tccu))
					{
					$xyz = md5("$x$i");
					mysql_query("INSERT INTO siswa_uang_lab (kd, kd_siswa, kd_tapel, kd_kelas, bln, thn) VALUES ".
		...
		if ($tcc != 0)
			...
			Pembayaran :
			<br>
			<input name="bayar" type="text" value="Uang lab" size="30" class="input" readonly>
			</p>
