...
if ($_POST['btnSMP'])
	...
	$majalah = nosql($_POST['majalah']);
	$barkode = nosql($_POST['barkode']);
	$judul = cegah2($_POST['judul']);
	$volume = cegah2($_POST['volume']);
	$nomor = cegah2($_POST['nomor']);
	...
	$terbit_tgl = nosql($_POST['e_terbit_tgl']);
	$terbit_bln = nosql($_POST['e_terbit_bln']);
	$terbit_thn = nosql($_POST['e_terbit_thn']);
	$tgl_terbit = "$terbit_thn:$terbit_bln:$terbit_tgl";
	...
	if ($s == "baru")
		{
		///cek
		$qcc = mysql_query("SELECT * FROM perpus_item2 ".
		...
		if ($tcc != 0)
			...
			{
			//query
			mysql_query("INSERT INTO perpus_item2(kd, barkode) VALUES ".
					"('$kd', '$barkode')");
	...
	//jika update
		...
		if (($tcc != 0) AND ($cc_kd != $kd))
			...
			{
			//query
			mysql_query("UPDATE perpus_item2 SET kd_perolehan = '$perolehan', ".
					"kd_majalah = '$majalah', ".
					"rak = '$rak', ".
					"kode = '$kode', ".
					"barkode = '$barkode', ".
					"topik = '$judul', ".
					"tglterbit = '$tgl_terbit', ".
					"tglmasuk = '$tgl_masuk', ".
					"tglentri = '$today', ".
					"volume = '$volume', ".
					"nomor = '$nomor', ".
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$limit;$i++)
		...
		//del
		mysql_query("DELETE FROM perpus_item2 ".
				"WHERE kd = '$kd'");
		}
	...
	if (empty($filex_namex))
		...
		if ($ext_filex == ".jpg")
			...
			if (!file_exists($path1))
				...
				//query
				mysql_query("UPDATE perpus_item2 SET img_cover = '$filex_namex' ".
						"WHERE kd = '$kd'");
...
if (($s == "baru") OR ($s == "edit"))
	...
	$qx = mysql_query("SELECT DATE_FORMAT(tglmasuk, '%d') AS masuk_tgl, ".
				...
				"DATE_FORMAT(tglterbit, '%d') AS terbit_tgl, ".
				"DATE_FORMAT(tglterbit, '%m') AS terbit_bln, ".
				"DATE_FORMAT(tglterbit, '%Y') AS terbit_thn, ".
				"perpus_item2.* ".
				"FROM perpus_item2 ".
				"WHERE kd = '$kd'");
	...
	$e_volume = balikin2($rowx['volume']);
	$e_nomor = balikin2($rowx['nomor']);
	$e_masuk_tgl = nosql($rowx['masuk_tgl']);
	$e_masuk_bln = nosql($rowx['masuk_bln']);
	$e_masuk_thn = nosql($rowx['masuk_thn']);
	$e_terbit_tgl = nosql($rowx['terbit_tgl']);
	$e_terbit_bln = nosql($rowx['terbit_bln']);
	$e_terbit_thn = nosql($rowx['terbit_thn']);
	...
	//majalah
	$qoleh = mysql_query("SELECT * FROM perpus_m_majalah2 ".
				"WHERE kd = '$e_kd_majalah'");
	$roleh = mysql_fetch_assoc($qoleh);
	$e_majalah = balikin2($roleh['nama']);
	...
	Nama Majalah :
	<br>
	<select name="majalah" '.$x_enter.'>
	<option value="'.$e_kd_majalah.'" selected>'.$e_majalah.'</option>';

	//list
	$qrak = mysql_query("SELECT * FROM perpus_m_majalah2 ".
				"ORDER BY nama ASC");
	...
	do
		...
		$rak_nama = balikin($rrak['nama']);

		echo '<option value="'.$rak_kd.'">'.$rak_nama.'</option>';
		}
	...
	Volume :
	<br>
	<input name="volume" type="text" value="'.$e_volume.'" size="10"  '.$x_enter.'>
	</p>

	<p>
	Nomor :
	<br>
	<input name="nomor" type="text" value="'.$e_nomor.'" size="10"  '.$x_enter.'>
	</p>

	<p>
	Tgl. Terbit :
	<br>
	<input name="e_terbit_tgl" type="text" value="'.$e_terbit_tgl.'" size="2" '.$x_enter.'>
	<input name="e_terbit_bln" type="text" value="'.$e_terbit_bln.'" size="2" '.$x_enter.'>
	<input name="e_terbit_thn" type="text" value="'.$e_terbit_thn.'" size="4" '.$x_enter.'>
	</p>
