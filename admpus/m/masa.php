...
if ($_POST['btnSMP'])
	...
	if ($s == "baru")
		...
		if ((empty($kode)) OR (empty($nama)))
			...
			{
			///cek
			$qcc = mysql_query("SELECT * FROM perpus_m_masa ".
						"WHERE kode = '$kode'");
			...
			if ($tcc != 0)
				...
				//re-direct
				$pesan = "Kode Masa : $kode, Sudah Ada. Silahkan Ganti Yang Lain...!!";
				$ke = "$filenya?s=baru";
				pekem($pesan,$ke);
				exit();
				}
				...
				{
				//query
				mysql_query("INSERT INTO perpus_m_masa(kd, kode, nama) VALUES ".
						"('$kd', '$kode', '$nama')");
	...
	//jika update
		...
		{
		//query
		mysql_query("UPDATE perpus_m_masa SET kode = '$kode', ".
				"nama = '$nama' ".
				"WHERE kd = '$kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysql_query("DELETE FROM perpus_m_masa ".
				"WHERE kd = '$kd'");
		}
...
if (($s == "baru") OR ($s == "edit"))
	...
	$sqlcount = "SELECT * FROM perpus_m_masa ".
