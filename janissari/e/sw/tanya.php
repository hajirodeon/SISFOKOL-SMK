...
//nek mapel telah dipilih
	...
	if ($tpel != 0)
		...
		//jika daftar tanya
			...
			$sqlcount = "SELECT DISTINCT(guru_mapel_tanya.kd) AS nkd ".
					"FROM guru_mapel_tanya ".
					"WHERE guru_mapel_tanya.kd_guru_mapel = '$gmkd' ".
			...
			if ($count != 0)
				...
				do
					...
					//detail
					$qku = mysql_query("SELECT guru_mapel_tanya.* ".
								"FROM guru_mapel_tanya ".
								"WHERE guru_mapel_tanya.kd = '$d_kd' ".
								"AND guru_mapel_tanya.dari = '$kd1_session'");
					$rku = mysql_fetch_assoc($qku);
					$d_tanya = balikin($rku['tanya']);
					$d_jawaban = balikin($rku['jawaban']);
					$d_postdate = $rku['postdate'];
